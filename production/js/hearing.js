...
const mainImg = document.getElementById("main-woman");
const mainImg2 = document.getElementById("main-circle");
...
const state = {
  ...
  step: 0,
  activeTab: -1,
  tabName: ["災前準備", "災時應變", "災後復原", "協助聽障者", "文件下載"],
  sectionsTitle: [
    [
      "建立個人緊急聯絡網",
      "共同討論緊急應變計畫",
...
prepare.addEventListener("click", async () => {
  ...
  tl.to(
    tabs,
    { backgroundColor: "#F2F2F2", color: "#005671", duration: 0.5 },
    "-=1",
  );
  // tl.to(
  //   ".section-tabs",
  ...
  tl.to(
    ...
    { backgroundColor: "#005671", color: "#fff", duration: 0.5 },
    "-=1",
  );
  tl.to(
  ...
  // tl.to("#progress", { backgroundColor: state.activeTab % 2 == 1 ? "#D9F1FA" : "#F2F2F2", duration: .3 },
  //   "-=1")
  // sidebar
...
response.addEventListener("click", async () => {
  ...
  tl.to(
    tabs,
    { backgroundColor: "#D9F1FA", color: "#005671", duration: 0.5 },
    "-=1",
  );
  tl.to(
    tabs[state.activeTab],
...
tabs.forEach((tab, i) =>
  tab.addEventListener("click", async (e) => {
    ...
    if (clickedRecent) {
      ...
      // tl.to(main_1,{backgroundColor:state.activeTab%2==1?"#D9F1FA": "#F2F2F2",duration:.2},"-=0.25")
      switch (true) {
